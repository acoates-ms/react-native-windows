...
struct D2DHelpers {
 public:
  static void PushOpacityLayer(ID2D1DeviceContext *deviceContext, ID2D1Geometry *clipPathGeometry, float opacity) {
  ...
  static Numerics::float3x2 FromD2DTransform(D2D1_MATRIX_3X2_F const transform) {
    return Numerics::float3x2(transform.m11, transform.m12, transform.m21, transform.m22, transform._31, transform._32);
  }
