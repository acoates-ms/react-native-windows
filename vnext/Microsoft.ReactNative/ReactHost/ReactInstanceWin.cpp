...
void ReactInstanceWin::LoadBundle(std::string &&path) noexcept {
  m_jsMessageThread.Load()->runOnQueue([weakThis = Mso::WeakPtr{this}, p = std::move(path)]() mutable {
    if (auto strongThis = weakThis.GetStrongPtr()) {
      ...
      try {
        ...
        if (!PathFileExistsA(p.c_str())) {
          GetErrorCallback()("LoadBundle: Failed to find bundle: " + p);
          return;
        }
#if defined(_CHAKRACORE_H_)
        auto bundleString = FileMappingBigString::fromPath(p);
#else
        auto bundleString = JSBigFileString::fromPath(p);
#endif
        m_innerInstance->loadScriptFromString(std::move(bundleString), std::move(p), true);
#else
        auto bundleString = std::make_unique<::react::uwp::StorageFileBigString>(p);
        instance->loadScriptFromString(std::move(bundleString), std::move(p), true);
      }
#if defined(_CHAKRACORE_H_)
      catch (const facebook::react::ChakraJSException &e) {
        GetErrorCallback()(std::string{e.what()} + "\r\n" + e.getStack());
        return;
      }
#endif
      catch (std::exception &e) {
        strongThis->GetErrorCallback()(e.what());
        return;
      } catch (winrt::hresult_error const &e) {

        std::ostringstream ss;
        ss << "LoadBundle: [" << std::hex << e.code() << std::dec << "] ";
        ...
        if (SUCCEEDED(restricted->GetErrorDetails(&description, &error, &restrictedDesc, &capabilitySid))) {
          ss << Microsoft::Common::Unicode::Utf16ToUtf8(description) << ": "
              << Microsoft::Common::Unicode::Utf16ToUtf8(restrictedDesc);
        } else {
          ss << Microsoft::Common::Unicode::Utf16ToUtf8(e.message());
        }
        SysFreeString(description);
        SysFreeString(restrictedDesc);
        SysFreeString(capabilitySid);

        strongThis->GetErrorCallback()(ss.str());
#endif
      }
    }
  });
...
void ReactInstanceWin::InitUIMessageThread() noexcept {
    // Native queue was already given us in constructor.
    m_uiQueue =
        winrt::Microsoft::ReactNative::implementation::ReactDispatcher::GetUIDispatchQueue(m_options.Properties);
